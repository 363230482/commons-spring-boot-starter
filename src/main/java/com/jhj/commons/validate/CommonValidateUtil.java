package com.jhj.commons.validate;


import com.jhj.commons.date.DateOperator;
...
public final class CommonValidateUtil {
	...
	/**行政区域字符串*/
	public static final String DISTRICT_STRINGS = "京津沪渝黑吉辽冀豫鲁晋湘鄂粤桂云贵川陕内宁甘新青藏皖赣苏浙闽琼";
	
	/**数字正则表达式,不包含小数*/
	public static final String DIGITAL_REG_EXP = "\\d";
	...
	/**手机号码正则表达式*/
	public static final String MOBILE_REG_EXP = "^0?(13|15|18|14|17)[0-9]{9}$";
	
	/**座机号码正则表达式*/
	public static final String TEL_REG_EXP = "^(0\\d{2,3}-?)?\\d{7,8}$";
	
	/**电子邮箱正则表达式*/
	public static final String EMAIL_REG_EXP = "^\\w+((-\\w+)|(\\.\\w+))*\\@[A-Za-z0-9]+((\\.|-)[A-Za-z0-9]+)*\\.[A-Za-z0-9]+$";
		
	/**车牌号码正则表达式*/
	public static final String CAR_PLATE_NO_REG_EXP = "^[" + DISTRICT_STRINGS + "]{1}[A-Za-z]{1}[A-Za-z0-9]{5}$";
	
	/**证件有效期正则表达式*/
	public static final String PAPER_VALI_PERIOD = "^\\d{4}-\\d{2}-\\d{2}至\\d{4}-\\d{2}-\\d{2}$";
	 ...
	 * 
	 * @param str
	 * @param minLength minimum value : 1
	 * @param maxLength minimum value : minLength
	 * @return
	 */
	 ...
	 * <br />
	 * 
	 * @exception IllegalArgumentException 1.when minimum length equal or less than zero 2.maximum length less than minimum length
	 * 
	 * @param digital
	 * @param minLength
	 * @param maxLength
